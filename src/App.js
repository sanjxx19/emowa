  ...
  import { User, Heart, MessageCircle, TrendingUp, BarChart3, Users, Settings, Plus, Send, Eye, EyeOff, AlertTriangle, Smile, Meh, Frown, Zap } from 'lucide-react';
  ...
  const RegisterForm = ({ onRegister, onSwitchToLogin }) => {
    const [formData, setFormData] = useState({ user_name: '', user_email: '', password: '' });
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        await api.register(formData.user_name, formData.user_email, formData.password);
        onRegister();
      } catch (err) {
          ...
          {analysis && showAnalysis && (
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getSentimentColor(analysis.sentiment?.sentiment_label)}`}>
                  {getSentimentIcon(analysis.sentiment?.sentiment_label)}
                  {analysis.sentiment?.sentiment_label} ({Math.round(analysis.sentiment?.confidence * 100)}%)
                </span>
          ...
          {(post.sentiment_label || post.is_sarcastic) && (
            <div className="flex flex-wrap gap-1">
              {post.sentiment_label && (
                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getSentimentColor(post.sentiment_label)}`}>
  ...
  const SentimentFilter = ({ currentFilter, onFilterChange }) => {
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg p-4 mb-6">
        <h3 className="text-sm font-medium text-gray-700 mb-3">Filter by Sentiment</h3>
        <div className="flex flex-wrap gap-2">
          {filters.map((filter) => (
            <button
  ...
  const AnalyticsDashboard = () => {
    ...
    const totalSentiments = analytics.sentiment_distribution?.reduce((sum, item) => sum + item.count, 0) || 0;
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="space-y-2">
              {analytics.sentiment_distribution?.map((item) => {
                const percentage = totalSentiments > 0 ? (item.count / totalSentiments) * 100 : 0;
                return (
          ...
          <div className="border-t pt-4">
            <h4 className="text-sm font-medium text-gray-700 mb-3">Sarcasm Statistics</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-lg p-3">
                <div className="text-2xl font-bold text-gray-900">{analytics.sarcasm_stats?.total_analyzed || 0}</div>
                <div className="text-sm text-gray-600">Total Analyzed</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-3">
                <div className="text-2xl font-bold text-purple-600">{analytics.sarcasm_stats?.sarcasm_percentage || 0}%</div>
  ...
  const MainContent = ({ view, onPostCreated }) => {
    ...
    return (
      <div className="space-y-6">
        ...
        <SentimentFilter currentFilter={sentimentFilter} onFilterChange={setSentimentFilter} />
